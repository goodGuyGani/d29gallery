...
$query_category="SELECT * FROM orders WHERE orders.USER_ID = '$user_id' ORDER BY ORDER_ID DESC LIMIT $start, $limit";

    $result1 = $conn->query("SELECT count(ORDER_ID) AS ORDER_ID FROM orders WHERE orders.USER_ID = '$user_id'");
        ...
        {
            echo '<h1 align="Center" style="margin-top:300px;margin-bottom:300px;color:white">No Orders</h1>';
        }
        else{
        while($row=mysqli_fetch_array($result_category)){
...
if($row['ORDER_STATUS'] == 'Pending'){
    ...
    <div class="align">
        ...
        <div class="panel" style="padding-left:30px">
                ...
                <a style="text-decoration:none;color:white" href="cancel_order.php?id='.$row['ORDER_ID'].'&user='.$user_id.'"onclick="return(YNconfirm());"><button type="button" name="cancel_btn" class="soldbutton" style="width:80%;font-size:14px;height:50px" >CANCEL ITEMS</button></a>
                <br>
...
<div class="pagination" style="">
  <a href="orders.php?page=<?= $Previous; ?>">&laquo;</a>
  
  <?php for($i = 1; $i <= $pages; $i++) : ?>
  <a href="orders.php?page=<?= $i; ?>"><?= $i; ?></a>
  <?php endfor; ?>
  
  <a href="orders.php?page=<?= $Next; ?>">&raquo;</a>
