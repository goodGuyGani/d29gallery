...
$query_category="SELECT * FROM orders WHERE orders.USER_ID = '$user_id' AND orders.ORDER_STATUS = 'Cancelled' ORDER BY ORDER_ID DESC LIMIT $start, $limit";

    $result1 = $conn->query("SELECT count(ORDER_ID) AS ORDER_ID FROM orders WHERE orders.USER_ID = '$user_id' AND orders.ORDER_STATUS = 'Cancelled'");
        ...
        {
            echo '<h1 align="Center" style="margin-top:300px;margin-bottom:300px;color:white">No Cancelled Orders</h1>';
        }
        else{
        while($row=mysqli_fetch_array($result_category)){
...
<div class="pagination" style="">
  <a href="orders_cancelled.php.php?page=<?= $Previous; ?>">&laquo;</a>
  
  <?php for($i = 1; $i <= $pages; $i++) : ?>
  <a href="orders_cancelled.php?page=<?= $i; ?>"><?= $i; ?></a>
  <?php endfor; ?>
  
  <a href="orders_cancelled.php?page=<?= $Next; ?>">&raquo;</a>
