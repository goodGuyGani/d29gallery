...
<div class="card shadow mb-4">
  <div class="card-header py-3">
    <h6 class="m-0 font-weight-bold text-primary">Shipped Orders
    </h6>
...
<!--
  ...
  <div class="card-body">
    ...
    $query = "SELECT * FROM user,orders
                        WHERE user.USER_ID = orders.USER_ID AND orders.ORDER_STATUS = 'Shipping' LIMIT $start, $limit ";
    $query_run = mysqli_query($conn, $query);

    $result1 = $conn->query("SELECT count(ORDER_ID) AS ORDER_ID FROM orders WHERE ORDER_STATUS = 'Shipping' ");
...
<nav aria-label="Page navigation example">
  <ul class="pagination">
    <li class="page-item"><a class="page-link" href="admin_orders1(shipped).php?page=<?= $Previous; ?>">Previous</a></li>
    <?php for($i = 1; $i <= $pages; $i++) : ?>
      <li class="page-item"><a class="page-link" href="admin_orders1(shipped).php?page=<?= $i; ?>"><?= $i; ?></a></li>
    <?php endfor; ?>
    <li class="page-item"><a class="page-link" href="admin_orders1(shipped).php?page=<?= $Next; ?>">Next</a></li>
  </ul>
</nav>
