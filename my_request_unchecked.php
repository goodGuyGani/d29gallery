...
$query_category="SELECT * FROM commission_request WHERE commission_request.USER_ID = '$user_id' AND commission_request.COM_STATUS = 'Unchecked' ORDER BY COM_ID DESC LIMIT $start, $limit";

$result1 = $conn->query("SELECT count(COM_ID) AS COM_ID FROM commission_request WHERE commission_request.USER_ID = '$user_id' AND commission_request.COM_STATUS = 'Unchecked'");
        ...
        {
            echo '<h1 align="Center" style="margin-top:300px;margin-bottom:300px;color:white">No Unchecked Request</h1>';
        }
        else{
        while($row=mysqli_fetch_array($result_category)){
...
<div class="pagination" style="">
  <a href="my_request_unchecked.php?page=<?= $Previous; ?>">&laquo;</a>
  
  <?php for($i = 1; $i <= $pages; $i++) : ?>
  <a href="my_request_unchecked.php?page=<?= $i; ?>"><?= $i; ?></a>
  <?php endfor; ?>
  
  <a href="my_request_unchecked.php?page=<?= $Next; ?>">&raquo;</a>
