...
<h3 class="heading" style="font-family: Georgia, 'Times New Roman', Times, serif;margin-top:30px"> <span>My Sold Artworks</span></h3>
...
$id = $_SESSION['USER_ID'];
            ...
            $query_category="SELECT art_work.art_id, art_work.art_title,art_work.art_price, user.user_fname, user.user_mname,user.user_lname,art_work.art_description,art_work.art_imagepath,art_work.art_category
                         FROM art_work,user
                        where art_work.user_id = user.user_id AND user.user_id = '$id' AND art_work.art_status = 'Sold' ORDER BY rand()";
...
<!--
    <div class="img">
        ...
        <?php
$id = $_SESSION['USER_ID'];
echo '<p><h1 class="head-available"> Sold </h1>
                <h1 class="head-artworks">Artworks</p>';
  echo                '<a class="addbtn" href="select_art.php" style=""> &#10133;  Add an Artwork</a>';
            $query_category="SELECT art_work.art_id, art_work.art_title,art_work.art_price, user.user_fname, user.user_mname,user.user_lname,art_work.art_description,art_work.art_imagepath,art_work.art_category
                         FROM art_work,user
                        where art_work.user_id = user.user_id AND user.user_id = '$id' AND art_work.art_status = 'Sold' ORDER BY art_work.art_title ASC";
            $result_category = mysqli_query($conn,$query_category);
 if(mysqli_num_rows($result_category) <=0)
        {
            echo '<br><br><br><h1 align="Center">No Sold Artworks </h1>';
        }
        else{
            while($row = mysqli_fetch_array($result_category)){
